...
**Status (2026-09-15):** Local scheduler running. Send lead **10 min** (OC-19). Daily Live snapshot (OC-18). Lot-alumni audience (OC-15) lists `CollectorNotify` every 15 min; dashboard **Will notify N · EverOwnedLot M**. Next proof: a `source=datastore` MOMENT (Sandbox tonight only if that user has no `moment_days` row today; Live **~15:47 UTC** tomorrow). Spec: `roblox_gacha/docs/COLLECTOR_OPEN_CLOUD_NOTIFICATIONS.md`.
...
| OC-01 | P0 | Clock module | Implement from Lua `SpecialNpcDirector.hash32` / golden table, **not** the TCG spec “mod 2^32” sentence. Golden: `2026-09-10T16` → hash `1396388120`, jitter **433**, `slotUnix` `1789056433`, `pushAt` `1789055833` (600s lead). `Math.imul` path must fail. **Done.** |
...
| OC-06 | P1 | Config + secrets | `config/schedule.json` committed (`hoursLocal` + `notifyHoursLocal`). `.env.example` placeholders. Keys never in git **or GitHub Actions**. `.gitignore` in PR-1. **Done.** |
| OC-07 | P1 | Allowlist audience | `config/allowlist.sandbox.json` / `allowlist.live.json`. Fallback when `CollectorNotify` is missing. Empty list ⇒ no HTTP unless a datastore cache exists. **Done.** |
...
| OC-13 | P1 | First Sandbox allowlist send | **Done** (200s from 2026-09-10). |
| OC-14 | P2 | Gated Live allowlist | Tiny Live list + `LIVE_SENDS_ENABLED`. Notify hour **16**. **Ops in place**; first Live HTTP still the 16:00 UTC window. |
| OC-15 | P2 | DataStore audience (lot alumni) | See **OC-15 contract**. List `CollectorNotify`; Open Cloud list id is `global/<userId>`. 14-day recency; 15 min cache; skip list in the 60s window. Fallback to allowlist if store missing. Empty alumni ⇒ no HTTP. Dashboard: notify N / EverOwnedLot M. **Done** (keys set; both universes list). |
...
| OC-19 | P1 | Push lead 8 → 10 min | `pushLeadSeconds` **600**. Golden `2026-09-10T16` `pushAt` **1789055833** (was 1789055953). Send window still 60s. Dashboard copy “about 10 minutes” (edited in place). `slotUnix` / jitter / toast / catch unchanged. **Done.** |
...
| PR-3b | Push lead 600s + dashboard copy | OC-19 **Done** (Dashboard string edited in place) |
| PR-4 | Fly always-on (later) | OC-11 |
| PR-5 | DataStore audience (lot alumni) | OC-15 **Done.** TCG published sandbox + live. |
...
| Decision | Value |
| --- | --- |
| Who | **Post-ship lot alumni** — `CollectorNotify` keys TCG writes after `EverOwnedLot` shipped. **No historical backfill.** Not CCU. |
| Recency | `updatedUnix >= nowUnix − 14 × 86400` |
| Re-add | Post-ship alumni who **join again** get a fresh `updatedUnix` and re-enter the 14-day window without claiming again. Pre-ship owners are invisible until they claim once after ship. |
| Store | Standard DataStore `CollectorNotify` **in that universe**. Engine key = `tostring(userId)`. Open Cloud list `id` is `global/<userId>` (path `.../scopes/global/entries/<userId>`). Value **must** include `updatedUnix`. Do **not** key by `lotId`. |
| List vs send | List/filter **outside** the 60s window (15 min cache in sqlite `audience_cache`). Window is HTTP only. Dashboard shows notify N vs listed EverOwnedLot. |
...
| TCG-OC-09 | P1 | `PushLeadSeconds` 10 min | **Done** (with OC-19). `10 * 60`. Do **not** change jitter, toast, catch, or `VisitHoursUtc`. |
